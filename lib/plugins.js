...
var debug = require('debug')('component-proxy:plugins');
var path = require('path');

var resolve = require('./resolve');
var utils = require('./utils');

var replace = utils.replaceStrings;
...
exports.resolveDependency = function () {
  return function* resolveDependency(next) {
    ...
    this.uri = utils.localPath.apply(null, res);
    this.remoteURI = utils.remotePath.apply(null, res);
...
function validate(dependency) {
  ...
  debug('got %s', uri);
  // full remote name
  return dependency.remoteURI = utils.localToRemotePath(uri);
}
