...
function* walk(entrypoint, dependencies) {
  ...
  function* rewrite(next) {
    ...
    if (/\bfunction\s+require\s*\(/.test(string)) {
      file.log('normalize:common:info', '`function require(){}` detected, opting out of JS rewriting')
      return
    }
  ...
  function* overwrite(filename, path) {
    ...
    if (isRelative(path)) {
      // check aliases
      var target = dependencies[resolve(folder, sanitizeRelative(path))]
      if (target) {
      ...
      for (var i = 0; i < extensions.length; i++) {
        ...
        debug('looking up %s', target)
        if (yield fs.exists(target)) {
          debug('target %s exists', target)
      ...
      if (target === false) return builtins.cluster // because `cluster` is empty
